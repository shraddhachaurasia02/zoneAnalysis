        ...
        for b in base_range:
            for l_o in legout_range:
                ...
                for idx in indices:
                    ...
                    if label_name == "Drop-Base-Rally": zone_low = min(zone_low, stock.iloc[idx][l])
...
def scan_stock(TICKER, PERIOD, htf_interval, htf_pattern, htf_base_count, htf_legout_count,
    ...
    if enable_confluence and not htf_stock.empty:
        _, htf_result = find_demand_zones(htf_stock, TICKER, htf_pattern, htf_base_count, htf_legout_count, htf_legin_thresh, htf_legout_thresh, htf_bs_thresh, htf_strict_mode, htf_buffer, htf_base_mode, htf_legout_mode, htf_enable_entry_filter, htf_zone_status, htf_marking_type)
    ...
    ltf_stock, ltf_result = find_demand_zones(ltf_stock, TICKER, ltf_pattern, ltf_base_count, ltf_legout_count, ltf_legin_thresh, ltf_legout_thresh, ltf_bs_thresh, ltf_strict_mode, ltf_buffer, ltf_base_mode, ltf_legout_mode, ltf_enable_entry_filter, ltf_zone_status, ltf_marking_type)
    ...
    if enable_confluence:
        ...
        for _, ltf_z in ltf_unique.iterrows():
            matches = htf_zones[(htf_zones['Zone_Low'] <= ltf_z['Zone_Low']) & (ltf_z['Zone_High'] <= htf_zones['Zone_High'])]
