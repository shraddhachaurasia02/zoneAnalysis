...
# We use ticker names directly for labels now to support 1000+ stocks efficiently
...
<style>
   ...
   * { font-family: 'Outfit', sans-serif; letter-spacing: -0.02em; }
   ...
   .main { background: linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%); padding: 0; }
   .block-container { padding: 2rem 3rem; max-width: 1600px; }
   .glass-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px); border-radius: 24px; padding: 2rem; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.1); border: 1px solid rgba(255, 255, 255, 0.18); margin-bottom: 1.5rem; }
   .main-header { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(20px); border-radius: 20px; padding: 1.5rem 2rem; margin-bottom: 2rem; border: 1px solid rgba(255, 255, 255, 0.2); }
   h1 { color: white !important; font-weight: 800 !important; font-size: 3rem !important; margin: 0 !important; }
   .stButton > button { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 16px; padding: 0.875rem 2rem; font-weight: 600; transition: all 0.3s ease; }
   button[kind="primary"] { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%) !important; }
   .metric-card { background: rgba(255, 255, 255, 0.95); border-radius: 20px; padding: 1.5rem; text-align: center; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.08); }
   .price-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 24px; padding: 2rem; color: white; margin-bottom: 1.5rem; }
</style>
...
# ================= HEADER =================
st.markdown(f"""<div class="main-header"><h1>📈 Institutional Zone Scanner</h1><p style="color: rgba(255,255,255,0.9); font-size: 1.1rem; margin: 0.5rem 0 0 0; font-weight: 500;">Universe: {selected_group_name} | Precision Supply & Demand Analysis</p></div>""", unsafe_allow_html=True)
...
if run_btn:
   ...
   for i, ticker in enumerate(selected_tickers):
       ...
       stock, result, error = scan_stock(ticker, PERIOD, INTERVAL, PATTERN, BASE_COUNT, num_legouts=LEGOUT_COUNT, legin_threshold=LEGIN_THRESH, legout_threshold=LEGOUT_THRESH, base_threshold=BS_THRESH, strict_mode=STRICT_MODE, entry_buffer_pct=BUFFER, base_mode=ENGINE_BASE_MODE, legout_mode=ENGINE_LEGOUT_MODE, enable_entry_filter=ENABLE_ENTRY_FILTER, zone_status_limit=ZONE_STATUS, marking_type=MARKING_TYPE)

       if result is not None:
           res_u = result.drop_duplicates(subset=['Formation_ID'])
           for _, p_slice in res_u.iterrows():
               findings.append({
                   "Ticker": ticker, "Company": company, "Pattern": p_slice['Pattern_Found'],
                   "Bases": p_slice['Base_Count'], "Leg-Outs": p_slice['LegOut_Count'],
                   "Zone High": round(p_slice['Zone_High'], 2), "Zone Low": round(p_slice['Zone_Low'], 2),
                   "Current Price": round(stock[f"Close_{ticker}"].iloc[-1], 2),
                   "Leg-In Date": p_slice['LegIn_Date'], "Leg-Out Date": p_slice['LegOut_Date'],
                   "Tests": p_slice['Tests'], "Formation_ID": p_slice['Formation_ID']
               })
       bar.progress((i + 1) / len(selected_tickers))

   status_text.success("✅ Scan complete!")
   if not findings:
       st.markdown("""<div class="glass-card" style="text-align:center;"><div style="font-size:2.5rem;">📭</div><h3>No Zones Detected</h3></div>""", unsafe_allow_html=True)
   else:
       ...
       st.session_state["scan_results"], st.session_state.ticker_index = df, 0
       c1, c2, c3, c4 = st.columns(4)
       m_data = [("Total Patterns", len(df), "🎯"), ("Unique Stocks", df['Ticker'].nunique(), "📊"), ("RBR", df['Pattern'].str.contains('Rally').sum(), "🟢"), ("DBR", df['Pattern'].str.contains('Drop').sum(), "🟡")]
       for col, (l, v, icon) in zip([c1, c2, c3, c4], m_data):
           with col: st.markdown(f"""<div class="metric-card"><div style="font-size: 2rem;">{icon}</div><div style="font-size: 2rem; font-weight: 800; color: #764ba2;">{v}</div><div style="color: #64748b; font-size: 0.85rem;">{l}</div></div>""", unsafe_allow_html=True)
...
if "scan_results" in st.session_state and not st.session_state["scan_results"].empty:
   ...
   with tab2:
       ...
       with cp: 
           if st.button("⬅️ Previous"): st.session_state.ticker_index = max(0, st.session_state.ticker_index-1); st.rerun()
       with cn: 
           if st.button("Next ➡️"): st.session_state.ticker_index = min(len(ticker_list)-1, st.session_state.ticker_index+1); st.rerun()
       with cs:
           sel_tick = st.selectbox("Select stock", options=ticker_list, index=st.session_state.ticker_index, label_visibility="collapsed")
           st.session_state.ticker_index = ticker_list.index(sel_tick)

       stock, result, error = scan_stock(sel_tick, PERIOD, INTERVAL, PATTERN, BASE_COUNT, num_legouts=LEGOUT_COUNT, legin_threshold=LEGIN_THRESH, legout_threshold=LEGOUT_THRESH, base_threshold=BS_THRESH, strict_mode=STRICT_MODE, entry_buffer_pct=BUFFER, base_mode=ENGINE_BASE_MODE, legout_mode=ENGINE_LEGOUT_MODE, enable_entry_filter=ENABLE_ENTRY_FILTER, zone_status_limit=ZONE_STATUS, marking_type=MARKING_TYPE)
       if stock is not None:
           last = stock[f"Close_{sel_tick}"].iloc[-1]
           st.markdown(f"""<div class="price-card"><h2>{sel_tick.replace(".NS", "")}</h2><div style="font-size: 2.5rem; font-weight: 800;">₹{last:,.2f}</div></div>""", unsafe_allow_html=True)
           ...
           fig.add_hline(y=last, line_dash="dash", line_color="#FF4B4B", annotation_text=f"CMP: {last:.2f}")
           ...
           if result is not None:
               ...
               for _, p in res_d.iterrows():
                   ...
                   fig.add_scatter(x=[p.name], y=[stock.loc[p.name, f"Low_{sel_tick}"] * 0.99], mode="markers+text", text=[f"{p['Pattern_Found']} ({p['Tests']} Tests)"], textposition="bottom center", marker=dict(symbol="triangle-up", size=16, color=marker_color))
           
           fig.update_layout(template="plotly_white", height=700, xaxis_rangeslider_visible=False, dragmode='pan', hovermode='x unified', xaxis=dict(fixedrange=False), yaxis=dict(fixedrange=False))
